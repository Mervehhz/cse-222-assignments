...
public class Street{
    ...
    /**
     * Constructor initializes building ArrayLists with street length, and initializes street positions to false because of there is no building
    ...
    public Street(int length){
        ...
        playGrounds = new ArrayList<>(LENGTH);
        leftPositions = new boolean[LENGTH/2];
        rightPositions = new boolean[LENGTH/2];
        houses = new ArrayList<>(LENGTH);
        markets = new ArrayList<>(LENGTH);
        offices = new ArrayList<>(LENGTH);
    ...
    /**
     * adds new building to ArrayList according to type of building
     * @param building building that will add
     */
    ...
    /**
     * deletes building from ArrayList according to type of building
    ...
    public Building isThereBuilding(int x){
        ...
        for(House temp : houses){
            if(temp.getPosition() <= x && x <= temp.getPosition() + temp.getLENGTH()){
                if(!flag) {
                    building = temp;
                    flag = true;
                }
